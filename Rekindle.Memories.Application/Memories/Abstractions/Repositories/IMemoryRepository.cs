...
{
    Task InsertMemory(Memory memory, CancellationToken cancellationToken = default, ITransactionContext? transactionContext = null);
    Task<Memory?> FindById(Guid memoryId, CancellationToken cancellationToken = default);
    Task<IEnumerable<Memory>> FindByGroupId(Guid groupId, int limit, DateTime? cursor = null, CancellationToken cancellationToken = default);
